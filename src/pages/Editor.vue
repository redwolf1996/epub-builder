...
<template>
  <div class="editor-page h-full flex" :class="{ 'is-fullscreen': isFullscreen }">
    ...
    <main class="flex-1 flex flex-col min-w-0 min-h-0 overflow-hidden" @click="handleEditorAreaClick">
      <EditorToolbar
        :editor-ref="editorActions"
        :exporting="exporting || ocrProcessing"
        :show-chapter-toggle="showDrawerToggle"
        :chapter-toggle-active="showChapterDrawer"
        :sync-scroll="syncScroll"
        :compact="isMobile"
        @export="handleExport"
        @ocr="handleOcr"
        @fullscreen="toggleFullscreen"
        @toggle-chapter="showChapterDrawer = !showChapterDrawer"
      ...
      <div ref="splitContainerRef" class="split-container flex-1 flex min-h-0 overflow-hidden">
        <div
          ...
          :style="{ width: isMobile ? '100%' : `${editorRatio * 100}%` }"
        ...
        <div
          class="min-h-0 overflow-hidden"
          :style="{ width: isMobile ? '100%' : `${(1 - editorRatio) * 100}%` }"
          :class="{ hidden: isMobile && !editorStore.previewMode }"
        >
      ...
      <div class="editor-statusbar flex items-center justify-between gap-3 px-3 shrink-0">
        <span class="text-xs status-counts">
          {{ t('editor.wordCount', { count: editorStore.wordCount }) }} · {{ t('editor.charCount', { count: editorStore.charCount }) }}
        </span>
      ...
      <div v-if="isMobile" class="mobile-tabs flex">
        <button
          class="mobile-tab flex-1 py-2 text-sm text-center transition-colors"
          :class="{ active: !editorStore.previewMode }"
          @click="editorStore.previewMode = false"
        >
        ...
        <button
          class="mobile-tab flex-1 py-2 text-sm text-center transition-colors"
          :class="{ active: editorStore.previewMode }"
          @click="editorStore.previewMode = true"
        >
